...
public class LoadTickersTask extends DbTask<List<Ticker>> {

    public static final String TAG = "com.luischavezb.bitso.assistant.android.tag.LOAD_TICKERS_TASK";

    public LoadTickersTask(boolean enableDialog, int... targets) {
        super(TAG, enableDialog, false, targets);
    }

    @Override
    protected List<Ticker> execute(DbHelper dbHelper) {
        Context context = AssistantApplication.getContext();

        publishProgress(context.getString(R.string.progress_get_tickers));

        return dbHelper.readTickers();
    }
}
