...
public class LoadBalancesTask extends DbTask<List<Balance>> {

    public static final String TAG = "com.luischavezb.bitso.assistant.android.tag.LOAD_BALANCES_TASK";

    public LoadBalancesTask(boolean enableDialog, int... targets) {
        super(TAG, enableDialog, false, targets);
    }

    @Override
    protected List<Balance> execute(DbHelper dbHelper) {
        Context context = AssistantApplication.getContext();

        publishProgress(context.getString(R.string.progress_get_balances));

        return dbHelper.readBalances();
    }
}
